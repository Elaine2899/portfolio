...
const EN = {
    ...
    'pbr.prep': 'Chunks and embeds 29 lectures of Whisper statistics transcripts into Supabase pgvector (halfvec, 50% storage saved), served to Telegram by a Supabase Edge Function — always-on Q&A, quizzes, and summaries, prototyped on n8n and hand-rewritten when the trial ended — with retrieval quality measured on a self-built 16-question eval set.',
    ...
    // PrepAgent case study (pa.*)
    'pa.tech': 'Tech stack: Supabase Edge Functions (Deno/TypeScript) · Supabase Postgres + pgvector · Python · Google Gemini · Telegram Bot API · n8n (v1)',
    'pa.nature': '<strong>Type:</strong> Solo project | fully cloud-native, always-on',
    'pa.h1': 'Motivation & Problem Definition',
    'pa.s1p1': 'This project grew out of a real personal need: I\'m preparing for graduate school entrance exams, statistics is mandatory, and my main study material is NCTU professor Li-Ying Tang\'s YouTube statistics lectures — solid content, but video can\'t be asked questions, and it fits poorly into fragmented review time. Rather than stay a viewer, I decided to turn the course into a study assistant I could talk to anytime, hitting three goals at once: <strong>hands-on practice with RAG and vector databases, solving my own exam-prep need, and validating what n8n cloud workflows can do</strong>.',
    'pa.s1p2': 'The positioning differs from FunCtrl: FunCtrl was a team capstone solving "<strong>locating</strong> knowledge points in an open video library"; PrepAgent is a solo-built <strong>closed-corpus Q&A system</strong> — a fixed corpus (29 lectures of one course), depth-first, aiming to turn one course into a knowledge base you can converse with. There is also a technical through-line: the speech-to-text work FunCtrl never got to try became step one here — all 29 lectures transcribed with Whisper before anything else could exist.',
    'pa.s1p3': 'Exam prep dictated two hard constraints: it\'s a long campaign, so the service must be <strong>always-on</strong> without depending on my own laptop — hence fully cloud-native; and entrance exams cover more than statistics, so the architecture must be <strong>modular</strong> for new subjects to plug in cheaply (expansion already underway).',
    'pa.h2': 'Method Selection & Architecture Evolution',
    'pa.s2p1': 'The early selection logic: spend effort where the learning is.',
    'pa.s2l1': '<li><strong>n8n Cloud</strong>: validating low-code workflows was a goal in itself — visual nodes wired up the whole "Telegram webhook → retrieval → prompt assembly → Gemini generation" flow into a working version fast.</li><li><strong>Telegram</strong>: the lowest-friction way to ship a bot (free official Bot API, solid docs) — and the bot was never the point, so the chat layer got minimal effort, saving energy for the RAG.</li><li><strong>Supabase (Postgres + pgvector)</strong>: vectors, conversation memory, and subject settings live in one managed database — nothing to self-host, free tier sufficient.</li>',
    'pa.s2p2': '<strong>Evolution: n8n expired, a hand-written backend took over.</strong> When the n8n Cloud free trial ended, I rewrote the whole flow as a Supabase Edge Function — one <code>telegram-bot</code> function handling webhook, routing, RAG, and Gemini calls. The proven RAG flow stayed intact; only two things changed: the query embedding model was swapped, and the system prompt went from hardcoded "statistics / Prof. Tang" to being driven by subject settings (paving the way for multi-subject expansion). The migration meant building the same system twice, low-code and hand-written: n8n starts fast, but plan limits are a real cost; the hand-written version bought full control and zero monthly fees.',
    'pa.h3': 'System Design',
    'pa.h3a': 'Fully cloud-native architecture',
    'pa.s3l1': '<li><strong>Compute</strong>: one Supabase Edge Function (Deno runtime, TypeScript) carries all service logic — Telegram webhook entry, command routing, RAG retrieval, Gemini calls.</li><li><strong>Data</strong>: three tables in one Supabase Postgres, each with its job — <code>documents</code> (pgvector store), <code>chat_memory</code> (conversation memory), <code>chat_settings</code> (subject config).</li><li><strong>Keep-alive</strong>: Supabase\'s built-in pg_cron + pg_net self-ping once a day, preventing the free plan from pausing on idle.</li><li><strong>Only two external dependencies</strong>: Telegram (message entry via webhook into the Edge Function) and the Google Gemini API (LLM generation + embeddings, called over raw REST).</li>',
    'pa.s3p1': 'The only local piece is a Python harness for one-off batch work — cleaning, chunking, embedding and loading the Whisper transcripts (ingestion), plus retrieval evaluation — not a resident service. <strong>Laptop off, service on</strong>: the "always-on" constraint from the Motivation section, delivered.',
    'pa.h3b': 'Three key design decisions',
    'pa.s3l2': '<li><strong>Raw REST, no SDK — one principle across both runtimes</strong>: Google\'s official SDK defaults to <code>v1beta</code> binding under AI Studio API keys and kept hitting version conflicts. So the Python ingestion side (<code>ingest.py</code>, <code>eval_retrieval.py</code>) uses <code>requests</code> and the Edge Function side (<code>index.ts</code>) uses native <code>fetch</code> — both calling Gemini\'s REST endpoints directly: fewer dependencies, identical and controllable behavior on both ends.</li><li><strong>halfvec to break the dimension ceiling</strong>: <code>gemini-embedding-001</code> outputs 3072 dimensions, past the 2000-dimension ivfflat index limit of pgvector\'s standard <code>vector</code> type. Switching to <code>halfvec</code> (16-bit floats) raised the index ceiling to 4000 dimensions — index built — while saving 50% storage and memory, with precision loss negligible for exam-prep Q&A.</li><li><strong>Sliding-window chunking</strong>: <code>chunker.py</code> first strips the "line number + tab" prefix from Whisper transcripts, then chunks at 20 lines with a 5-line overlap, preserving context across chunk boundaries so knowledge points don\'t fracture at retrieval time.</li>',
    'pa.h3c': 'Conversation memory & citations',
    'pa.s3p2': 'The bot keeps context via <code>chat_memory</code>: ask "what does lecture 2 cover", then just type <code>/quiz</code> — the bot quizzes you on lecture 2 without re-specifying. Every answer cites its source lecture, spot-checked and mostly correct — anchoring trust with citations, against RAG\'s most common criticism of being unverifiable.',
    'pa.f1c': '/ask — auto-summarizes key points by lecture',
    'pa.f2c': '/quiz — continues quizzing on lecture 2 from conversation memory',
    'pa.h4': 'Retrieval Quality Evaluation',
    'pa.s4p1': 'This is the lesson carried over from FunCtrl — <strong>build the evaluation before optimizing</strong>; with no teammates to keep quality honest, the principle matters even more solo. Evaluation runs via <code>eval_retrieval.py</code> in the local harness, fully repeatable:',
    'pa.s4l1': '<li><strong>Eval set</strong>: 16 statistics questions spanning basic and advanced lectures; each question\'s ground-truth lecture was established by keyword-scanning the transcripts.</li><li><strong>The eval path is identical to the live bot</strong> (<code>RETRIEVAL_QUERY</code> embedding → <code>match_documents</code>) — it measures the retrieval quality users actually experience, not a lab number.</li><li><strong>Result</strong>: Hit Rate@5 = <strong>75% (12/16)</strong>; of the 12 hits, <strong>11 ranked the correct lecture #1</strong>.</li>',
    'pa.s4p2': 'More important than 75% is knowing why the 4 misses missed. Isolation tests showed: for those 4 questions (ANOVA, standard deviation/variance, central tendency, discrete/continuous random variables), the best similarity inside the correct lecture was only 0.61–0.65, below the 0.65–0.76 of the wrong lectures — a <strong>real semantic gap</strong>, not a near-miss in ranking. Two causes: basic lectures are highly colloquial, working through examples rather than textbook-style definitions that the queries resemble; and the ANOVA query was absorbed by ANOVA-table content in the regression lectures — a reasonable confusion. The corresponding fixes are in the next section.',
    'pa.h5': 'Limitations & Future Work',
    'pa.s5l1': '<li><strong>The real semantic gap</strong>: the colloquial-transcript vs definitional-query problem from the miss analysis has three known remedies — hybrid search (keyword + vector), contextual chunking (prefixing chunks with lecture topics), query rewriting — not yet implemented, and the main axis of the next iteration.</li><li><strong>Formula transcription loss</strong>: Whisper cannot render spoken math as LaTeX, so formula-dense passages read poorly and their chunks embed poorly — a corpus ceiling specific to math-heavy subjects like statistics.</li><li><strong>Evaluation scale</strong>: 16 questions, with ground truth built by keyword scanning — both coverage and labeling method leave room for bias; growing the question bank and manually verifying ground truth are improvements to the evaluation itself.</li><li><strong>Single corpus</strong>: one course, one lecturer\'s speaking style. Whether retrieval parameters and prompts overfit this course won\'t be known until a second subject plugs in — multi-subject expansion is underway (the <code>chat_settings</code> design exists for exactly this).</li><li><strong>Free-tier fragility</strong>: the pg_cron self-ping prevents idle pauses, but free-plan ceilings (Edge Function runtime, database size) remain; if users grow beyond one, the cost model needs a redesign.</li>',
    'pa.h6': 'Reflections',
    'pa.s6p1': 'This project turned FunCtrl\'s "evaluation first" lesson into my standard method, and the miss analysis pushed it further — 75% itself isn\'t the point; knowing which 4 questions lost, and that they lost to a semantic gap rather than ranking, is the map for the next iteration. The other takeaway is building the same system twice, low-code then hand-written: n8n excels at validating a flow quickly, and once validated, migrating to hand-written code was cheap — because the flow was already proven. "Map it in low-code, then pave it in code" is now my default path for side projects. What I\'d change on a rerun: keep process artifacts — the n8n-era workflow retired without screenshots, so that chapter can only be told in words now.',
